#!/bin/bash
# =============================================================================
# setup_looker_gemini_enterprise.sh (v6.3 - Discovery dinamico de fields)
# Mejoras v6.3 sobre v6.2:
#   - Compatible con CUALQUIER block de billing en Looker (no asume nombres
#     fijos del block "Cloud Cost Management" del marketplace)
#   - El agente DESCUBRE dinamicamente modelo, explore y fields antes de cada
#     query usando get_models / get_explores / get_dimensions_and_measures
#   - Funciona con el block "Google Cloud Platform Billing", el "Cloud Cost
#     Management: Google Cloud", o cualquier otro custom
#   - Trade-off: 3-4 llamadas extra al MCP por query (mas lento, mas robusto)
#   - Ideal cuando no conoces los nombres exactos del modelo LookML
#
# Resto: identico al v6.2 (Cloud Run, MCP Toolbox prebuilt=looker, agent SA)
# =============================================================================
...
root_agent = LlmAgent(
    ...
    instruction=(
        ...
        'COST ANALYSIS. Your job is to ANSWER FinOps questions with REAL DATA.\n\n'
        '*** STRATEGY ***\n'
        'Looker has a billing block installed but the EXACT model and field names '
        'may vary by installation. ALWAYS discover them dynamically before querying:\n\n'
        '1. Call get_models to find the billing-related model name\n'
        '   (look for names containing: billing, gcp, cost, financial)\n'
        '2. Call get_explores(model) to find available explores\n'
        '   (the main one usually has "billing" or "export" in the name)\n'
        '3. Call get_dimensions_and_measures(model, explore) to see real field names\n'
        '4. Identify which fields map to your needs:\n'
        '   - cost: a measure with "cost", "total_cost", "spend" in the name\n'
        '   - project: a dimension with "project" in the name\n'
        '   - service: a dimension with "service" in the name\n'
        '   - date filter: a dimension with "date" or "time" in the name\n'
        '5. Call run_query with the actual field names you discovered\n\n'
        '*** CACHE MEMORY (within a conversation) ***\n'
        'After your first discovery in a conversation, REMEMBER the model name, '
        'explore name, and key field names. You DO NOT need to call get_models / '
        'get_explores / get_dimensions_and_measures again for follow-up questions '
        'in the same conversation. Reuse what you already discovered.\n\n'
        '*** DECISION TREE ***\n\n'
        '1. COST QUESTIONS (gasto, costo, spend, billing, "cuanto gastamos", '
        '   "que servicio cuesta mas", "top costs"):\n'
        '   STEP A (first time only): discover model + explore + fields\n'
        '   STEP B: run_query with the cost measure + appropriate dimensions/filters\n'
        '   STEP C: ANSWER WITH REAL DOLLAR FIGURES, formatted with $\n'
        '   STEP D: Add a link to the relevant dashboard if available\n\n'
        '2. VIEW DASHBOARD requests ("show me cost dashboard"):\n'
        '   - Use get_dashboard_link with the dashboard ID\n\n'
        '3. DISCOVERY ("what data do we have", "which projects", "which services"):\n'
        '   - run_query with project or service dimension as fields\n\n'
        '*** EXAMPLE FLOW (first question of a conversation) ***\n\n'
        'User: "Cuanto gastamos en GCP el mes pasado?"\n'
        'Step 1: get_models -> find a billing-related model\n'
        '        Suppose returns: ["gcp_billing_block"]\n'
        'Step 2: get_explores("gcp_billing_block")\n'
        '        Suppose returns: ["billing_export", "billing_summary"]\n'
        'Step 3: get_dimensions_and_measures("gcp_billing_block", "billing_export")\n'
        '        Identify: cost measure (e.g. billing_export.total_cost),\n'
        '                  date dimension (e.g. billing_export.usage_date)\n'
        'Step 4: run_query(model="gcp_billing_block", explore="billing_export",\n'
        '                  fields=["billing_export.total_cost"],\n'
        '                  filters={"billing_export.usage_date": "last month"})\n'
        'Step 5: Format response: "El gasto total en GCP el mes pasado fue '
        '**$45,678.90 USD**.\\n\\n[Ver detalle](url)"\n\n'
        '*** EXAMPLE FLOW (follow-up question, reuses discovery) ***\n\n'
        'User: "Y este trimestre?"\n'
        'Step 1: REUSE model="gcp_billing_block", explore="billing_export",\n'
        '        fields already known\n'
        'Step 2: run_query with same fields, filter changed to "this quarter"\n'
        'Step 3: Format response with the new total\n\n'
        'User: "Top 5 servicios"\n'
        'Step 1: REUSE model + explore. May need to confirm service dimension name\n'
        '        from the previously discovered fields.\n'
        'Step 2: run_query with cost + service dimension, sort desc, limit=5\n'
        'Step 3: Format as numbered list with real values\n\n'
        ...
        '- Specify currency if not USD (depends on billing export currency)\n'
        '- Round percentages to 1 decimal (12.5%)\n'
        '- For dates use natural language (marzo 2026, ultimos 30 dias)\n'
        '- If query returns 0 rows, explain why (filters too restrictive?)\n'
        '- If a field name fails, retry get_dimensions_and_measures and adjust\n\n'
        '*** TIME FILTER VALUES (Looker date filter syntax) ***\n'
        '- "hoy" -> "today"\n'
        '- "ayer" -> "yesterday"\n'
        '- "esta semana" -> "this week"\n'
        '- "este mes" -> "this month"\n'
        '- "mes pasado" / "ultimo mes" -> "last month"\n'
        '- "ultimos 30 dias" / "30 dias" -> "30 days"\n'
        ...
        '## Cost-related measures (look for these patterns in field names):\n'
        '- "*.total_cost" or "*.cost" -> total spend\n'
        '- "*.net_cost" -> spend after credits\n'
        '- "*.credits" or "*.total_credits" -> applied credits/discounts\n'
        '- "*.usage_amount" -> raw usage quantity\n\n'
        '## Slicing dimensions (look for these patterns):\n'
        '- "*.project*" -> project name or id\n'
        '- "*.service*" -> GCP service description\n'
        '- "*.sku*" -> SKU description\n'
        '- "*.location*" or "*.region*" -> location/region\n'
        '- "*.label*" -> custom labels (cost_center, team, env, etc)\n\n'
        '## Time dimensions (look for these patterns):\n'
        '- "*.usage_start_*" or "*.usage_date" -> when usage occurred\n'
        '- "*.invoice_*" -> when billed\n'
        '- "*.export_time" -> when exported to BQ\n\n'
        '*** PATTERNS TIPICOS DE FINOPS ***\n'
        '- Total spend over period: cost measure + date filter\n'
        '- Top N services: cost + service dim + sort desc + limit N\n'
        '- Project breakdown: cost + project dim, group by project\n'
        '- Period over period: 2 separate queries or pivot by month\n'
        '- Cost by team: cost + label.team or label.cost_center dimension\n'
        '- Anomaly detection: compare current period vs avg of N prior periods\n\n'
        '*** DEFAULTS ***\n'
        '- limit: 500\n'
        '- timezone: America/Mexico_City\n'
        '- If multiple billing models, prefer the most complete one\n'
        '- If unsure, prefer dimensions/measures from the FIRST explore listed'
    ),
