#!/bin/bash
# =============================================================================
# setup_looker_gemini_enterprise.sh (v7 - FINAL base64 inline)
# Arquitectura simplificada (sin GCS, sin firma, sin impersonation):
...
#   4. Dashboards renderizados como PNG -> base64 -> data URI -> markdown inline
#   5. Imagenes embebidas en el mensaje (no dependen de URLs externas)
#
# Por que base64 en vez de GCS:
#   - Gemini Enterprise NO renderiza imagenes desde URLs externas (limitacion del frontend)
#   - Gemini Enterprise SI renderiza data URIs (porque son parte del mensaje)
#   - Elimina TODA la complejidad de buckets, firma, signBlob y permisos especiales
#
# Trade-off:
#   - PNGs deben ser <2MB para que el frontend no corte el mensaje
#   - Por eso usamos dimensiones reducidas (900x600 dashboards, 800x500 looks)
# =============================================================================
...
REGION="us-central1"
BUCKET_NAME="YOUR_GCS_BUCKET_NAME"          # bucket staging del ADK (SOLO para deploy)
BUCKET_LOCATION="US"
...
echo ""
echo "=================================================="
echo " PASO 2: Service accounts y permisos (simplificado)"
echo "=================================================="
...
echo ""
echo "Asignando permisos al SA del Agente (solo lo esencial)..."
...
# 3. Lectura del bucket de staging (para descargar packages durante runtime si aplica)
gcloud projects add-iam-policy-binding "$PROJECT_ID" \
  --member="serviceAccount:${AGENT_SA}" \
  --role="roles/storage.objectViewer" \
  --condition=None &>/dev/null
echo "  [OK] storage.objectViewer (staging bucket)"

echo "Permisos minimos del SA del agente asignados."

echo ""
echo "=================================================="
echo " PASO 3: Bucket staging (UNICO bucket necesario)"
echo "=================================================="
...
# -----------------------------------------------------------------------------
# Convertir PNG a data URI base64 (embebible inline en markdown)
# -----------------------------------------------------------------------------
def _png_to_data_uri(png_bytes: bytes) -> str:
    """Convierte PNG bytes a data URI base64.

    Gemini Enterprise renderiza data URIs inline (no requiere URL externa).
    Si el PNG > 2MB el frontend puede cortar el mensaje, por eso las dimensiones
    de las imagenes son moderadas (900x600 / 800x500).
    """
...
# -----------------------------------------------------------------------------
# Signed SSO Embed URL para Looker (link interactivo)
# -----------------------------------------------------------------------------
...
def show_dashboard_inline(dashboard_id: str) -> dict:
    """Renderiza un dashboard de Looker como imagen inline en el chat.

    Args:
        dashboard_id: ID numerico del dashboard.
    Returns:
        Dict con markdown que incluye la imagen embebida + link interactivo.
    """
    sdk = looker_sdk.init40()
    ...
    try:
        data_uri = _png_to_data_uri(png_bytes)
    except Exception as e:
        return {"error": str(e), "markdown": f"Error al codificar imagen: {e}"}
    ...
    return {
        "dashboard_id": dashboard_id,
        "markdown": (
            f"![Dashboard {dashboard_id}]({data_uri})\n\n"
...
def show_look_inline(look_id: str) -> dict:
    """Renderiza un Look como imagen inline en el chat.
...
def show_query_inline(model: str, explore: str, fields: list, vis_type: str = "looker_column") -> dict:
    """Ejecuta query ad-hoc y muestra resultado como imagen inline.
    ...
    Returns:
        Dict con markdown que incluye la imagen embebida.
    """
    sdk = looker_sdk.init40()
...
root_agent = LlmAgent(
    ...
    name='looker_agent',
    description='Looker agent rendering dashboards as inline images via base64 data URIs.',
    instruction=(
        'You are a Looker data agent inside Gemini Enterprise. '
        'You render Looker dashboards and charts as INLINE IMAGES via base64 data URIs.\n\n'
        '*** CRITICAL OUTPUT RULE ***\n'
        'When a tool returns a "markdown" field, you MUST output that field EXACTLY AS-IS, '
        'character-by-character, including the entire base64 data URI. DO NOT:\n'
        '- Truncate or shorten the data URI\n'
        '- Replace the image with "image here" text\n'
        '- Rewrite the markdown in your own words\n'
        '- Add extra explanation before or after\n'
        '- Wrap content in code blocks\n'
        '- Split the data URI across lines\n\n'
        'The base64 data URI may be very long (hundreds of KB) - output it all anyway. '
        'It is the only way the image renders inline in Gemini Enterprise.\n\n'
        'TOOLS:\n'
...
echo ""
echo "=================================================="
echo " PASO 7: Deploy con Python SDK"
...
echo ""
echo "Las imagenes se renderizan inline via base64 data URIs"
echo "(no se usa GCS para imagenes - todo va embebido en el mensaje)"
echo ""
echo "Prueba con:"
